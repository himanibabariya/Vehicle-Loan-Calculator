      ...
      <div className='flex gap-x-10 flex-wrap max-w-7xl justify-center mx-auto mt-20'>
        <div className=''>
          <div className='my-6 relative'>
            ...
            <input placeholder='0' type="number" name="loanAmount" id="" value={loanAmount} onChange={(e) => setLoanAmount(Number(e.target.value))} className='h-9 border text-right border-gray-400 text-customRed hover:border-customRed focus:border-customRed' />
          </div>
          <hr className=' border-gray-400' />

          <div className='my-6'>
            <label htmlFor="downPayment" className='w-72 inline-block text-xl '>Down Payment</label>
            <input placeholder='0' type="number" name="downPayment" id="" value={downPayment} onChange={(e) => setDownPayment(Number(e.target.value))} className='h-9 border text-right border-gray-400 text-customRed ' />
          </div>
          <hr className='border-gray-400'/>

          <div className='my-6'>
            <label htmlFor="interestRate" className='w-72 inline-block text-xl'>Interest rate</label>
            <input placeholder='0.00' type="number" name="interestRate" id="" value={interestRate} onChange={(e) => setinterestRate(Number(e.target.value))} className='h-9 border text-right border-gray-400' />
          </div>
          <hr className='border-gray-400'/>

          <div className='my-6'>
            <label htmlFor="payoffMonths" className='w-72 inline-block text-xl'>Pay it off on...</label>
            <input type="number" name="payoffMonths" id="" value={payoffMonths}
              onChange={(e) => setPayoffMonths(Number(e.target.value))} className='h-9 w-24 text-right<i class="fa-solid fa-dollar-sign"></i> border border-gray-400' />
            <label htmlFor="" className='ml-6'>Months</label>
          </div>

          <input type="range" name="" id="" min="6" max='84' value={payoffMonths}
            onChange={(e) => setPayoffMonths(Number(e.target.value))} className='range-input' />
        </div>
